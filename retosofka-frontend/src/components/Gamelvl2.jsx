...
const Gamelvl2 = () => {
  ...
  const startGame = async e => {
    e.preventDefault();
    const res = await axios.get('http://localhost:8080/api/ask/ask/bycategory/2')
  ...
  const nextLevel = async e => {
    ...
    if (selectValue === ask.correctAnswer) {
      const url = `http://localhost:8080/api/player/updatescore/${userNameFromLocal}/${40}`
      await axios.put(url)
      new Swal({
        title: "Respuesta correcta!",
        text: "Serás redirigido al nivel 3",
        icon: "success"
      });
      navigate('/Gamelvl3')
  ...
  return (
    <Container>
      <div>
        <ButtonSide>
          <Button size="large" onClick={startGame} variant="contained" color="secondary" type="submit" >Iniciar juego nivel 2</Button>
        </ButtonSide>
        <Wrapper>
          <Title>{ask.ask} </Title>
          <FilterText>
