...
const Gamelvl3 = () => {
  ...
  const startGame = async e => {
    e.preventDefault();
    const res = await axios.get('http://localhost:8080/api/ask/ask/bycategory/3')
  ...
  const nextLevel = async e => {
    ...
    if (selectValue === ask.correctAnswer) {
      const url = `http://localhost:8080/api/player/updatescore/${userNameFromLocal}/${60}`
      await axios.put(url)
      new Swal({
        title: "Respuesta correcta!",
        text: "Serás redirigido al nivel 4",
        icon: "success"
      });
      navigate('/Gamelvl4')
  ...
  return (
    <Container>
      <div>
        <ButtonSide>
          <Button size="large" onClick={startGame} variant="contained" color="secondary" type="submit" >Iniciar juego nivel 3</Button>
        </ButtonSide>
        <Wrapper>
          <Title>{ask.ask} </Title>
          <FilterText>
